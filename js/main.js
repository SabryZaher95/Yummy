...
$("document").ready(function () {
  $(".lds-ripple").fadeOut(700, function () {
    $("#loading").fadeOut(300);
...
async function getDataByArea(area) {
  ...
  var Meal = "";
  var size = 20;
  if(meals.meals.length < 20){
    size = meals.meals.length;
  }
...
// I N G R E D I A N T S -- API FUNCTIONS //
  ...
  for (let i = 0; i < size; i++) {
    ...
    <div class="col-md-6 col-lg-3 my-3">
      <div class="rounded position-relative overflow-hidden">
        <div class="overflow-hidden pointer" onclick=" getMealsByIngrediants('${meals.meals[i].strIngredient}')">
          ...
          <p class="text-white overflow-hidden fs-6 mt-1 px-0 fw-lighter">
            ${meals.meals[i].strDescription.split(" ").splice(0, 20).join(" ")}
          </p>
        </div>
      </div>
    </div>`;
